...
for (p in 1:length(plate.list)) {
  f.path[p]<- paste0("/Users/habbasi/Desktop/050919_Nathianel_Gray/", plate.list[p], "/", plate.list[p], ".csv")
  metafile[p] <- paste0("/Users/habbasi/Desktop/050919_Nathianel_Gray/", plate.list[p], "/", plate.list[p], "_Metadata.csv")
  file_varselect[p] <- paste0("/Users/habbasi/Desktop/050919_Nathianel_Gray/", plate.list[p], "/", plate.list[p],"_normalized_variable_selected.csv")
  variables_imp <- as.data.frame(readr::read_csv(file_varselect[p]), stringsAsFactors =F)
  varlist <- colnames(variables_imp)[which(!str_detect(colnames(profile_varselect), "Metadata_"))]
...
metadata <- colnames(data)[which(str_detect(colnames(data), "Metadata_"))]
variables <- colnames(data)[which(!str_detect(colnames(data), "Metadata_"))]
...
df_meta <- data %>% select(metadata) %>% dplyr::collect()
profile  <- cbind(df_meta, data_scaled)

readr::write_csv(profile, paste0("/Users/habbasi/Desktop/050919_Nathianel_Gray/", plate.list[p], "/", plate.list[p], "_normalized_sampled_variable_selected.csv")) 
